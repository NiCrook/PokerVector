                        ...
                        if let Some(a) = first_aggressor_on_turn {
                            if a.player == player {
                                ...
                                if hero_saw_street(hand, player, Street::River) {
                                    result.river.0 = true;
                                    let first_aggressor_on_river = hand.actions.iter()
                                        .find(|a| a.street == Street::River && matches!(a.action_type, ActionType::Bet { .. }));
        ...
        if let Some(ref pfa_name) = pfa {
            ...
            if hero_saw_street(hand, player, Street::Turn) {
                let pfa_cbet_flop = hand.actions.iter()
                    .any(|a| a.street == Street::Flop && matches!(a.action_type, ActionType::Bet { .. }) && a.player == *pfa_name);
                if pfa_cbet_flop {
                    ...
                    if let Some(bet_action) = first_bet_turn {
                        if bet_action.player == *pfa_name {
                            ...
                            if pfa_cbet_turn && hero_saw_street(hand, player, Street::River) {
                                let first_bet_river = hand.actions.iter()
                                    .find(|a| a.street == Street::River && matches!(a.action_type, ActionType::Bet { .. }));
                                if let Some(bet_action) = first_bet_river {
                                    if bet_action.player == *pfa_name {
                                        let player_response = hand.actions.iter()
                                            .skip_while(|a| !(a.street == Street::River && a.player == *pfa_name && matches!(a.action_type, ActionType::Bet { .. })))
                                            .skip(1)
                                            .find(|a| a.street == Street::River && a.player == player);
                                        if let Some(resp) = player_response {
...
mod tests {
    ...
    fn test_cbet_river() {
        ...
        hand.actions = vec![
            ...
            Action { player: "Villain".to_string(), action_type: ActionType::Check, street: Street::Turn },
            Action { player: "Hero".to_string(), action_type: ActionType::Bet { amount: make_money(8.00), all_in: false }, street: Street::Turn },
            Action { player: "Villain".to_string(), action_type: ActionType::Call { amount: make_money(8.00), all_in: false }, street: Street::Turn },
            // River: c-bet
            Action { player: "Villain".to_string(), action_type: ActionType::Check, street: Street::River },
            Action { player: "Hero".to_string(), action_type: ActionType::Bet { amount: make_money(16.00), all_in: false }, street: Street::River },
    ...
    fn test_fold_to_cbet_river() {
        ...
        hand.actions = vec![
            ...
            Action { player: "Hero".to_string(), action_type: ActionType::Call { amount: make_money(8.00), all_in: false }, street: Street::Turn },
            // River: PFA c-bets, hero folds
            Action { player: "Villain".to_string(), action_type: ActionType::Bet { amount: make_money(16.00), all_in: false }, street: Street::River },
            Action { player: "Hero".to_string(), action_type: ActionType::Fold, street: Street::River },
        ];
        hand.board = vec![make_card('T', 'h'), make_card('5', 'd'), make_card('2', 'c'), make_card('7', 's'), make_card('3', 'h')];
