...
impl PokerVectorMcp {
    ...
    async fn get_range_analysis(
        ...
        struct ComboData {
            total: u64,
            open: u64,     // hero was first raiser
            three_bet: u64, // hero re-raised a raiser
            call: u64,     // hero called (cold call or limp-call)
            fold: u64,     // hero folded
            limp: u64,     // hero open-limped
            wins: u64,
            profit_bb: f64,
        }
        ...
        for hand in &hands {
            ...
            // Need exactly 2 hero cards for Hold'em combo
            if hand.hero_cards.len() != 2 { continue; }
            ...
            // Classify hero's preflop action
            // Walk through preflop actions to determine what hero did
            let mut raises_before_hero = 0u32;
            ...
            for action in &hand.actions {
                ...
                if action.player == hero {
                    if hero_action_found { continue; } // only first voluntary action
                    match &action.action_type {
                        ActionType::PostSmallBlind { .. }
                        | ActionType::PostBigBlind { .. }
                        | ActionType::PostAnte { .. }
                        | ActionType::PostBlind { .. } => {
                            // Not a voluntary action, skip
                        }
                        ActionType::Raise { .. } => {
        ...
        // Categorize combos into groups
        fn hand_category(combo: &str) -> &'static str {
            let chars: Vec<char> = combo.chars().collect();
            if chars.len() < 2 { return "other"; }

            let is_pair = chars.len() == 2; // e.g. "AA", "KK"
            ...
            // Check if both cards are broadways (T, J, Q, K, A)
            ...
            // Suited/offsuit connectors: adjacent ranks
